...
model = joblib.load('carfinal_bestfit_rf_model.pkl')

label_encoders = joblib.load('label_encoders.pkl')
scalers = joblib.load('scalers.pkl')

# Load dataset for filtering and identifying similar data
data = pd.read_csv('carfinal.csv')

# Set pandas option to handle future downcasting behavior
pd.set_option('future.no_silent_downcasting', True)

# Features used for training
features = ['ft', 'bt', 'km', 'transmission', 'ownerNo', 'oem', 'model', 'modelYear', 'Mileage', 'Seating Capacity', 'City','car_age', 'brand_popularity', 'mileage_normalized']
...
def filter_data(oem=None, model=None, body_type=None, fuel_type=None, seats=None):
    ...
    if seats:
        filtered_data = filtered_data[filtered_data['Seating Capacity'] == seats]
...
def preprocess_input(df):
    ...
    for column in ['ft', 'bt', 'transmission', 'oem', 'model', 'City']:
    ...
    for column in ['km', 'ownerNo', 'Mileage', 'Seating Capacity', 'modelYear']:
...
st.markdown("<h1 style='color: #337067;'>Predicted Market Value of Car</h1>", unsafe_allow_html=True)  # Title with custom color
st.markdown("<hr style='border: 2px solid #07b89e;'>", unsafe_allow_html=True)  # Horizontal line with custom color

# Sidebar for user inputs
st.sidebar.header('Input Car Features')

# Set background colors
input_background_color = '#07b89e;'  # Light maroon color
result_background_color = '#e7fff4;'  # Cosmic latte or beige color
...
st.markdown(
    ...
    .stSelectbox > div > div > div > select {{
        border: 2px solid #07b89e !important;
    }}
...
filtered_data = filter_data(oem=selected_oem, model=selected_model, body_type=body_type, fuel_type=fuel_type)
seat_count = visual_selectbox('6. Seating Capacity', filtered_data['Seating Capacity'].unique())
...
mileage = st.sidebar.slider('11. Mileage (kmpl)', min_value=min_mileage, max_value=max_mileage, value=min_mileage,
                            step=0.5)
        ...
        try:
            ...
            st.markdown(f"""
                <div class="result-container">
                    <h2 class="prediction-title">Predicted Valuation </h2>
